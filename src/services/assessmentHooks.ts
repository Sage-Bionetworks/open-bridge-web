...
export const useUpdateSurveyAssessment = () => {
  ...
  const mutation = useMutation<Assessment, Error, any, any>(update, {
    onMutate: async props => {
      ...
      const previousState = queryClient.getQueryData<Assessment[]>(
      ...
      switch (props.action) {
        case 'DELETE':
          ...
          break
        case 'COPY':
          newState.unshift(props.assessment)
          break
        case 'UPDATE':
          queryClient.setQueryData(
            ASSESSMENT_KEYS.list(appId, true, true),
            newState.map(a =>
              a.guid === props.assessment.guid ? props.assessment : a
            )
          )
          break
        case 'CREATE':
          queryClient.setQueryData(
            ASSESSMENT_KEYS.list(appId, true, true),
            newState.concat(props.assessment)
          )
          break
      }
