...
AssessmentLibraryFilterProps) => {
  ...
  const getTotalCount = (tags: object): number => Object.values(tags).reduce((a, c) => a + c, 0)
  ...
  const getFilteredAssessments = (assessments: Assessment[], tags: string[]) => {
    if (!tags.length) {
      return assessments
    } else {
  ...
  const changeTags = (isChecked: boolean, tag: string) => {
    const result = isChecked ? [...selectedTags, tag] : selectedTags.filter(item => item !== tag)
