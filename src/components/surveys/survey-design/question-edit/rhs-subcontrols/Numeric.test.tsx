...
test('renders the component with min and max', () => {
    renderComponent({ ...step, inputItem: { type: 'integer', fieldLabel: "My Label", formatOptions: { minimumValue: 3, maximumValue: 8 } } })
    const min = getMinInput()
    const max = getMaxInput()
...
test('updates values', async () => {
    const { user } = renderComponent({ ...step, inputItem: { type: 'integer', fieldLabel: "My Label", formatOptions: { minimumValue: 3, maximumValue: 8 } } })
    screen.debug()
    const min = getMinInput()
    const max = getMaxInput()
