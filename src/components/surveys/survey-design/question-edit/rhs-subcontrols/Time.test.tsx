...
test('renders the component with min and max', () => {
    renderComponent({ ...step, inputItem: { type: 'time', fieldLabel: "My Label", formatOptions: { minimumValue: '10:00', maximumValue: '12:00' } } })
    const min = getMinInput()
    const max = getMaxInput()
...
test('updates values', async () => {
    const { user } = renderComponent({ ...step, inputItem: { type: 'time', fieldLabel: "My Label", formatOptions: { allowFuture: false, allowPast: true, minimumValue: '10:00', maximumValue: '12:00' } } })
