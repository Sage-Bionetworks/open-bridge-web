...
import { makeStyles, Box, Button, InputLabel, MenuItem, Select } from '@material-ui/core'
...
const NewStudySessionContainer: FunctionComponent<NewStudySessionContainerProps> = ({
  ...
  const copySession = (event: React.ChangeEvent<{ value: unknown }>)=> {
    ...
    const assessments = sessions.find(session=> session.id === sessionId)?.assessments || []
    onAddSession([...assessments])
  }
  ...
  return (
    <Box className={clsx(classes.root)}>
      ...
      <InputLabel id="demo-simple-select-label">  +  Duplicate session</InputLabel>
        <Select
        ...
        >
          ...
          {sessions.map(session => (<MenuItem value={session.id} key = {session.id}>{session.name}</MenuItem>))}
