...
export const StyledSessionContainer = styled(Paper, {label: 'StyledSessionContainer'})(({theme}) => ({
...
const SessionCreator: FunctionComponent<SessionCreatorProps> = ({
  ...
  if (!study || !schedule?.sessions) {
    return <></>
  }

  return isReadOnly ? (
    <ReadOnlySessionCreator children={children} sessions={schedule.sessions} />
  ) : (
    <>
